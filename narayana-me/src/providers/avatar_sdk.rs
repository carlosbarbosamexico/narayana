//! Avatar SDK provider implementation
...
/// Avatar SDK provider (selfie-based avatars)
pub struct AvatarSDKProvider {
    config: AvatarConfig,
    api_key: String,
    base_url: String,
    client: Arc<Client>,
...
impl AvatarSDKProvider {
    /// Create a new Avatar SDK provider
    pub async fn new(config: AvatarConfig) -> Result<Self, AvatarError> {
        // Get API key from environment variable
        let api_key = std::env::var("AVATAR_SDK_API_KEY")
            .map_err(|_| AvatarError::Config("AVATAR_SDK_API_KEY environment variable not set".to_string()))?;
        ...
        let base_url = std::env::var("AVATAR_SDK_BASE_URL")
            .unwrap_or_else(|_| "https://api.avatarsdk.com/v1".to_string());
    ...
    /// Map expression to Avatar SDK format
...
#[async_trait]
impl AvatarProvider for AvatarSDKProvider {
    async fn initialize(&mut self, _config: &AvatarConfig) -> Result<(), AvatarError> {
        info!("Initializing Avatar SDK provider");
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| AvatarError::Api(format!("Failed to connect to Avatar SDK API: {}", e)))?;

        if !response.status().is_success() {
            warn!("Avatar SDK API health check returned: {}", response.status());
        }

        info!("Avatar SDK provider initialized");
        Ok(())
    }

    async fn start_stream(&mut self) -> Result<AvatarStream, AvatarError> {
        info!("Starting Avatar SDK stream");
        ...
        *self.ws_stream.write().await = Some(ws_stream);
        ...
        info!("Avatar SDK stream started: {}", stream_id);
        Ok(AvatarStream {
            stream_id,
            client_url,
            handle: Box::new(()),
    ...
    async fn stop_stream(&mut self) -> Result<(), AvatarError> {
        ...
        info!("Stopping Avatar SDK stream: {}", stream_id);
        ...
        info!("Avatar SDK stream stopped");
        Ok(())
    }
