...
impl BeyondPresenceProvider {
    /// Create a new Beyond Presence provider
    pub async fn new(config: AvatarConfig) -> Result<Self, AvatarError> {
        // Get API key from environment variable
        let api_key = std::env::var("BEYOND_PRESENCE_API_KEY")
            .map_err(|_| AvatarError::Config("BEYOND_PRESENCE_API_KEY environment variable not set".to_string()))?;
        ...
        let base_url = std::env::var("BEYOND_PRESENCE_BASE_URL")
            .unwrap_or_else(|_| "https://api.beyondpresence.ai/v1".to_string());
    ...
    /// Map expression to Beyond Presence format
    fn map_expression(&self, expression: &Expression) -> Result<String, AvatarError> {
        match expression {
            ...
            Expression::Thinking => Ok("contemplative".to_string()),
            Expression::Confused => Ok("confused".to_string()),
            Expression::Excited => Ok("excited".to_string()),
            Expression::Tired => Ok("tired".to_string()),
            Expression::Recognition => Ok("understanding".to_string()),
            Expression::Custom(s) => {
                // Validate custom expression string
...
#[async_trait]
impl AvatarProvider for BeyondPresenceProvider {
    async fn initialize(&mut self, _config: &AvatarConfig) -> Result<(), AvatarError> {
        info!("Initializing Beyond Presence provider");
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| AvatarError::Api(format!("Failed to connect to Beyond Presence API: {}", e)))?;

        if !response.status().is_success() {
            warn!("Beyond Presence API health check returned: {}", response.status());
        }

        info!("Beyond Presence provider initialized");
        Ok(())
    }

    async fn start_stream(&mut self) -> Result<AvatarStream, AvatarError> {
        info!("Starting Beyond Presence avatar stream");

        let stream_url = format!("{}/avatars/stream/create", self.base_url);

        // Validate and get avatar ID
        ...
        // For chunked responses or missing content-length, we'll validate after reading
        // by checking the actual JSON size
        ...
        // Store WebSocket stream and stream_id atomically to prevent leaks
        // If this write fails (unlikely), ws_stream will be dropped and connection closed
        *self.ws_stream.write().await = Some(ws_stream);
        self.stream_id = Some(stream_id.clone());

        // Create client URL (local bridge endpoint)
        let client_url = format!("ws://localhost:8081/avatar/stream/{}", stream_id);

        info!("Beyond Presence stream started: {}", stream_id);
        Ok(AvatarStream {
            stream_id,
            client_url,
            handle: Box::new(()),
    ...
    async fn stop_stream(&mut self) -> Result<(), AvatarError> {
        ...
        info!("Stopping Beyond Presence stream: {}", stream_id);
        ...
        // Stop stream via API (best effort)
        ...
        let stop_url = format!("{}/avatars/stream/{}/close", self.base_url, encoded_stream_id);
        let _ = self.client
            .post(&stop_url)
            .header("Authorization", format!("Bearer {}", self.api_key))
            .timeout(std::time::Duration::from_secs(10))
            .send()
            .await; // Best effort, don't fail if this errors

        info!("Beyond Presence stream stopped");
        Ok(())
    }
    ...
    async fn send_audio(&self, audio_data: Vec<u8>) -> Result<(), AvatarError> {
        ...
        if let Some(ref stream_id) = self.stream_id {
            ...
            // Use URL encoding to prevent injection attacks
            let encoded_stream_id = utf8_percent_encode(stream_id, NON_ALPHANUMERIC).to_string();
            let audio_url = format!("{}/avatars/stream/{}/audio", self.base_url, encoded_stream_id);
    ...
    async fn set_expression(&self, expression: Expression, intensity: f64) -> Result<(), AvatarError> {
        ...
        if let Some(ref stream_id) = self.stream_id {
            ...
            // Use URL encoding to prevent injection attacks
            let encoded_stream_id = utf8_percent_encode(stream_id, NON_ALPHANUMERIC).to_string();
            let expression_url = format!("{}/avatars/stream/{}/expression", self.base_url, encoded_stream_id);
        ...
        if let Some(ref stream_id) = self.stream_id {
            ...
            let gesture_id = match gesture {
                ...
                Gesture::Custom(ref s) => {
                    // Validate custom gesture string
            ...
            // Use URL encoding to prevent injection attacks
            let encoded_stream_id = utf8_percent_encode(stream_id, NON_ALPHANUMERIC).to_string();
            let gesture_url = format!("{}/avatars/stream/{}/gesture", self.base_url, encoded_stream_id);
    ...
    async fn update_emotion(&self, emotion: Emotion, intensity: f64) -> Result<(), AvatarError> {
        ...
        self.set_expression(expression, intensity).await
           ...
           async fn send_video_frame(&self, _frame_data: Vec<u8>, _width: u32, _height: u32) -> Result<(), AvatarError> {
               // Video frame processing not yet implemented for Beyond Presence
               Ok(())
           }

           async fn get_audio_output(&self) -> Result<Option<Vec<u8>>, AvatarError> {
               // TTS audio output not yet implemented for Beyond Presence
               Ok(None)
           }
