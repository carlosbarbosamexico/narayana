//! Open Avatar Chat provider implementation (open-source)
...
/// Open Avatar Chat provider (open-source)
pub struct OpenAvatarChatProvider {
    config: AvatarConfig,
...
impl OpenAvatarChatProvider {
    /// Create a new Open Avatar Chat provider
    pub async fn new(config: AvatarConfig) -> Result<Self, AvatarError> {
        // API key is optional for open-source provider
        let api_key = std::env::var("OPEN_AVATAR_CHAT_API_KEY").ok();
        ...
        let base_url = std::env::var("OPEN_AVATAR_CHAT_BASE_URL")
            .unwrap_or_else(|_| "https://api.openavatar.chat/v1".to_string());

        // Validate base URL
        if !base_url.starts_with("https://") && !base_url.starts_with("http://") {
            return Err(AvatarError::Config("Base URL must use HTTP or HTTPS".to_string()));
        }
    ...
    /// Map expression to Open Avatar Chat format
...
#[async_trait]
impl AvatarProvider for OpenAvatarChatProvider {
    async fn initialize(&mut self, _config: &AvatarConfig) -> Result<(), AvatarError> {
        info!("Initializing Open Avatar Chat provider");
        ...
        let response = request
            ...
            .send()
            .await
            .map_err(|e| AvatarError::Api(format!("Failed to connect to Open Avatar Chat API: {}", e)))?;

        if !response.status().is_success() {
            warn!("Open Avatar Chat API health check returned: {}", response.status());
        }

        info!("Open Avatar Chat provider initialized");
        Ok(())
    }

    async fn start_stream(&mut self) -> Result<AvatarStream, AvatarError> {
        info!("Starting Open Avatar Chat stream");
        ...
        *self.ws_stream.write().await = Some(ws_stream);
        ...
        info!("Open Avatar Chat stream started: {}", stream_id);
        Ok(AvatarStream {
            stream_id,
            client_url,
            handle: Box::new(()),
    ...
    async fn stop_stream(&mut self) -> Result<(), AvatarError> {
        ...
        info!("Stopping Open Avatar Chat stream: {}", stream_id);
        ...
        let mut request = self.client.post(&stop_url);
        ...
        info!("Open Avatar Chat stream stopped");
        Ok(())
    }
