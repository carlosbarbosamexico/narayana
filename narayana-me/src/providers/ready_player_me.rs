...
/// Ready Player Me provider
pub struct ReadyPlayerMeProvider {
    config: AvatarConfig,
    api_key: String,
    base_url: String,
    client: Arc<Client>,
...
impl ReadyPlayerMeProvider {
    /// Create a new Ready Player Me provider
    pub async fn new(config: AvatarConfig) -> Result<Self, AvatarError> {
        // Get API key from environment variable
        let api_key = std::env::var("READY_PLAYER_ME_API_KEY")
            .map_err(|_| AvatarError::Config("READY_PLAYER_ME_API_KEY environment variable not set".to_string()))?;
        ...
        let base_url = std::env::var("READY_PLAYER_ME_BASE_URL")
            .unwrap_or_else(|_| "https://api.readyplayer.me/v1".to_string());
    ...
    /// Map expression to Ready Player Me format
...
#[async_trait]
impl AvatarProvider for ReadyPlayerMeProvider {
    async fn initialize(&mut self, _config: &AvatarConfig) -> Result<(), AvatarError> {
        info!("Initializing Ready Player Me provider");
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| AvatarError::Api(format!("Failed to connect to Ready Player Me API: {}", e)))?;

        if !response.status().is_success() {
            warn!("Ready Player Me API health check returned: {}", response.status());
        }

        info!("Ready Player Me provider initialized");
        Ok(())
    }

    async fn start_stream(&mut self) -> Result<AvatarStream, AvatarError> {
        info!("Starting Ready Player Me stream");

        let stream_url = format!("{}/avatars/stream", self.base_url);
        ...
        let payload = serde_json::json!({
            "avatar_id": avatar_id,
            "lip_sync": self.config.enable_lip_sync,
            "expressions": true,
            "gestures": self.config.enable_gestures,
        });
        ...
        *self.ws_stream.write().await = Some(ws_stream);
        ...
        info!("Ready Player Me stream started: {}", stream_id);
        Ok(AvatarStream {
            stream_id,
            client_url,
            handle: Box::new(()),
    ...
    async fn stop_stream(&mut self) -> Result<(), AvatarError> {
        ...
        info!("Stopping Ready Player Me stream: {}", stream_id);
        ...
        info!("Ready Player Me stream stopped");
        Ok(())
    }
